...
public class ThreadedUDPClient implements Runnable {
	...
	public ThreadedUDPClient(String addr, int port) {
		try {
			...
			connection = new Connection(socket, InetAddress.getByName(addr), port, 0); //Create object
			this.init(); //Initialise the client
	 ...
	 */
	private void init() {
		process = new Thread(this, "server_process"); //Create process thread
		process.start(); //Start process thread
	}
